...
export function useStack() {
  ...
  const [settings, setSettings] = useLocalStorage<StackSettings>(
    "todo-stack-settings",
    {
      maxItems: 5,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setStack((currentStack) =>
        currentStack.filter(
          (item) => now - item.createdAt < settings.maxTimeInStack
        )
      );
  ...
  const moveItemUp = useCallback(() => {
    ...
    setStack((currentStack) => {
      const index = currentStack.findIndex(
  ...
  const deleteSelectedItem = useCallback(() => {
    ...
    setStack((currentStack) =>
      currentStack.filter((item) => item.id !== selectedItemId)
    );
    setSelectedItemId(null);
  }, [selectedItemId, setStack]);
